...
def test_log_solve_result_logs_params_metrics_and_artifacts(
    ...
    @contextmanager
    def fake_start_run(*, run_name: str, tags: dict[str, str]):
        yield MagicMock()

    fake_mlflow.start_run.side_effect = fake_start_run
    monkeypatch.setattr(tracking, "mlflow", fake_mlflow)
    ...
    assert start_run_kwargs["tags"]["family"] == "mip"
    assert start_run_kwargs["tags"]["status"] == "ok"
    assert start_run_kwargs["tags"]["solver"] == "gurobi"
